...
function NotificationItem({ notification, handleMarkAsRead }) {
  ...
  return (
    <Tooltip
      title="CẢNH BÁO NHIỆT ĐỘ VƯỢT MỨC QUY ĐỊNH !!!!"
      placement="top"
      followCursor
    >
      <ListItemButton
...
export default function NotificationsPopover() {
  ...
  const handleMarkAllAsRead = async () => {
    await axios
      .put(`${process.env.REACT_APP_URL}/doan1/email/update-email`, {
        reAddress: userAuth.email,
      })
      .then(() => {
        setNotifications(
          notifications.map((x) => {
  ...
  const handleMarkAsRead = async (obj) => {
    await axios
      .put(`${process.env.REACT_APP_URL}/doan1/email/update-email-by-id`, {
        _id: obj._id,
      })
      .then(() => {
        setNotifications(
