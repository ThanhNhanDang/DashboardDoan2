...
function ChartRelay2() {
  const data2 = useSelector((state) => state.relay2);
  const dataOn = [];
  const arrayDate = [];
  const arrayDate1 = [];
  let week = 1;
  ...
  arrayDate.every((item, index) => {
    ...
    if (data2[index].state === 'ON') relay2OnNumber += 1;
    return true;
  });
  dataOn.push(relay2OnNumber);
  const [dataRelay1Chart] = useState({
    series: [
      {
        name: 'Số lần bật',
        data: dataOn
  ...
  return (
    <Card>
      <CardHeader title="Biểu Đồ Trạng Thái Số Lần Bật Trong Một Tuần" subheader="Thiết Bị 2" />
